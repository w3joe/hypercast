...
describe('model size comparison', () => {
  ...
  it('includes zero-parameter models on a linear scale and keeps tied points accessible', () => {
    ...
    expect(screen.getAllByRole('button', { name: /total parameters/ })).toHaveLength(2)
  ...
  it('fits nearby points by default and can restore the full baseline scale', () => {
    const data = comparisonData([job('a', 100, [1.6, 1.62]), job('b', 110, [1.64, 1.66])], 32, 5, 'mae_ratio', true)
    render(<ParameterTradeoff data={data} metric="mae_ratio" />)
    const fit = screen.getByRole('button', { name: 'Fit points' })
    const baseline = screen.getByRole('button', { name: 'Include baseline' })
    expect(fit).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByText(/vertical scale is fitted/)).toBeVisible()
    fireEvent.click(baseline)
    expect(baseline).toHaveAttribute('aria-pressed', 'true')
    expect(fit).toHaveAttribute('aria-pressed', 'false')
    expect(screen.queryByText(/vertical scale is fitted/)).not.toBeInTheDocument()
  })
})
